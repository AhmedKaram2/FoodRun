...
export default function AdminApp({ language, user, onBack }) {
  ...
  return <main className="admin-shell">
    ...
    <nav className="admin-tabs">{['overview','users','rooms','orders','wallets','restaurants','settings'].map(value => <button className={tab === value ? 'active' : ''} onClick={() => setTab(value)} key={value}>{t(value[0].toUpperCase() + value.slice(1))}</button>)}</nav>
    ...
    <section className="admin-content">
      ...
      {tab === 'wallets' && <div className="card"><h2>{t("Wallet and settlement totals")}</h2>{dashboard.rooms.map(room => <section className="admin-wallet-room" key={room.id}><div className="admin-row wallet-admin"><span><b>{room.name} · #{room.orderNumber}</b><small>{room.payer ? `Payer: ${room.payer}` : 'Payer not selected'}</small></span><span><small>{t("Total")}</small><b>{money(room.totalMinor, room.currency)}</b></span><span><small>{t("Confirmed")}</small><b>{money(room.confirmedPaidMinor, room.currency)}</b></span><span><small>{t("Outstanding")}</small><b>{money(room.outstandingMinor, room.currency)}</b></span></div>{room.wallets.map(wallet => <div className="admin-wallet-person" key={wallet.memberId}><b>{wallet.name}</b><span>{t("Order")}{money(wallet.totalMinor, room.currency)}</span><span>{t("Paid")}{money(wallet.paidMinor, room.currency)}</span><span>{wallet.balanceMinor < 0 ? 'Refund' : 'Due'} {money(Math.abs(wallet.balanceMinor), room.currency)}</span></div>)}</section>)}</div>}
