...
class RoomDatabase(directory: File) : AutoCloseable {
    ...
    init {
        ...
        connection.createStatement().use { s ->
            ...
            require(version <= 2) { "This database requires a newer Food Run hub." }
    ...
    fun archive(room: Room) { connection.prepareStatement("INSERT OR REPLACE INTO orders VALUES(?,?,?)").use { it.setString(1, room.id); it.setLong(2, room.orderNumber); it.setString(3, encrypt(orderJson.encodeToString(room))); it.executeUpdate() } }
