...
internal class GroupPresentation(private val c: GroupController) {
    ...
    private fun feeFields() {
        ...
        if(delivery && c.page != GroupPage.SETUP) field(GroupFieldKey.AUTOMATIC_DELIVERY, tr("Use automatic delivery fee", "استخدام رسوم التوصيل التلقائية"), toggle = true)
        if(delivery && automatic) card("automatic-delivery", tr("Automatic delivery", "حساب التوصيل تلقائياً"), tr("AED 5 split equally for up to 5 people with food; AED 1 each for more than 5. Your own food counts too. Every receipt includes its share.", "٥ دراهم بالتساوي حتى ٥ أشخاص لديهم طعام، ودرهم لكل شخص عند أكثر من ٥. يُحسب طلبك أيضاً وتضاف الحصة لكل إيصال."))
