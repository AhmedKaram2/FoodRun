...
import { Page, loadRestaurants, storeRestaurants, blankRestaurant, normalizeRestaurant, clone, uid, minorInput, parseRestaurantExport, restaurantExport, downloadText, copyText } from './FoodRunApp';
  ...
  return <Page title={t("Restaurants & menus")} subtitle={t("Save restaurant details and prices once, or import the same Food Run schema used by the mobile apps.")} onBack={onBack}>
    <div className="library-layout">
      <aside className="card library-list">
        <div className="section-title compact"><div><p className="eyebrow">{t("SAVED ON THIS DEVICE")}</p><h3>{restaurants.length} {language === 'ar' ? 'مطاعم' : 'restaurants'}</h3></div><button className="icon-button" type="button" onClick={() => { setDraft(blankRestaurant()); setMessage(''); }}>＋</button></div>
      ...
      <form className="stack" onInvalid={event => { let node = event.target.parentElement; while (node) { if (node.tagName === 'DETAILS') node.open = true; node = node.parentElement; } }} onSubmit={save}>
        <section className="card editor-card stack">
          <div className="section-title compact"><div><p className="eyebrow">{t("RESTAURANT")}</p><h2>{draft.name || 'New restaurant'}</h2></div>{restaurants.some(item => item.id === draft.id) && <button type="button" className="link danger" onClick={() => { saveList(restaurants.filter(item => item.id !== draft.id)); setDraft(blankRestaurant()); }}>{t("Delete")}</button>}</div>
          ...
          <div className="form-grid three"><label>{language === 'ar' ? 'الإمارة' : 'Emirate'}<input value={draft.emirate || ''} onChange={e => set('emirate', e.target.value)} placeholder="Dubai" /></label><label>{language === 'ar' ? 'الإمارة بالعربية' : 'Arabic emirate'}<input dir="rtl" value={draft.emirateAr || ''} onChange={e => set('emirateAr', e.target.value)} placeholder="دبي" /></label><label>{language === 'ar' ? 'المنطقة' : 'Area'}<input value={draft.area || ''} onChange={e => set('area', e.target.value)} placeholder="Al Barsha" /></label><label>{language === 'ar' ? 'المنطقة بالعربية' : 'Arabic area'}<input dir="rtl" value={draft.areaAr || ''} onChange={e => set('areaAr', e.target.value)} placeholder="البرشاء" /></label><label>{language === 'ar' ? 'نوع المطبخ' : 'Cuisine'}<input value={draft.cuisine || ''} onChange={e => set('cuisine', e.target.value)} placeholder="Arabic" /></label></div>
          <div className="meal-editor"><b>{language === 'ar' ? 'الوجبات' : 'Meals'}</b>{[['breakfast', language === 'ar' ? 'فطور' : 'Breakfast'], ['lunch', language === 'ar' ? 'غداء' : 'Lunch'], ['dinner', language === 'ar' ? 'عشاء' : 'Dinner']].map(([value, label]) => <label className="check" key={value}><input type="checkbox" checked={draft.mealTypes.includes(value)} onChange={() => toggleMeal(value)} />{label}</label>)}</div>
