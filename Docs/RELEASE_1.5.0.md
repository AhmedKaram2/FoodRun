...
Browser push configuration was completed on 24 September 2026 using the project's public Web Push key supplied from Firebase Console. `VITE_FIREBASE_VAPID_KEY` is set in Netlify's production environment and the ignored local `webApp/.env.local`; the private key is not needed in the web app. The key passed P-256 public-key validation and the production web build passed.

Netlify deployment `6ab51cb9ade8343d7b238c56` is published at https://intrvioo.com. The live entry asset `/assets/index-ClVyWKqm.js` contains the configured public key and notification enablement code. Both `/notification-worker.js` and `/sw.js` were fetched successfully, including the push/click handlers and worker import. Users enable browser notifications from the notification inbox and accept their browser's permission prompt. Validation is limited to configuration, compilation and published-asset checks; end-to-end browser push delivery was not tested. Firebase documents key generation/import in the Console: https://firebase.google.com/docs/cloud-messaging/web/get-started.
