...
export function roomFeeUpdate(room, draft) {
  ...
  const automaticDelivery = room.deliveryMode && draft.automaticDelivery !== false;
  return {
    fees: { delivery: automaticDelivery ? 0 : amount(draft.delivery || '0', currency), automaticDelivery,
      service: amount(draft.service || '0', currency), discount: amount(draft.discount || '0', currency), proportionalDelivery: automaticDelivery ? false : draft.proportionalDelivery },
