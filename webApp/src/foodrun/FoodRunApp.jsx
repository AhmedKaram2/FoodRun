...
const ANDROID_DOWNLOAD_URL = 'https://github.com/AhmedKaram2/FoodRun/releases/download/v1.4.1/FoodRun-Android-1.4.1.apk';
  ...
  return <section id="app-downloads" className={`app-downloads ${compact ? 'compact' : ''}`}>
    ...
    <div className="download-grid">
      <article className="download-card">
        ...
        <div><strong>{t("Android app")}</strong><small>{t("Version 1.4.1 · Android 8+")}</small></div>
...
function RoomInviteCard({ room, hub }) {
  ...
  return <section className="card room-invite"><div className="invite-heading"><p className="eyebrow">{t("INVITE TO THIS ROOM")}</p><h3>{tx("Invite your group", "ادعُ مجموعتك")}</h3></div><div className="invite-qr"><QRCodeSVG value={link} size={164} level="M" marginSize={2} title={`Join ${room.name}`} /></div><div className="invite-details"><strong className="invite-code" aria-label={tx("Room code", "رمز الغرفة")}>{room.code}</strong><p className="muted">{tx("Scan the QR or share the link or code to join this room.", "امسح الرمز أو شارك الرابط أو الكود للانضمام إلى الغرفة.")}</p><div className="hero-actions"><button className="primary" onClick={() => copy(link, 'link')}>{copied === 'link' ? tx('✓ Link copied', '✓ تم نسخ الرابط') : tx('Copy invitation link', 'نسخ رابط الدعوة')}</button><button className="secondary" onClick={() => copy(room.code, 'code')}>{copied === 'code' ? tx('✓ Code copied', '✓ تم نسخ الكود') : tx('Copy code', 'نسخ الكود')}</button>{navigator.share && <button className="secondary" onClick={() => navigator.share({ title: `Join ${room.name} on Food Run`, text: `Room code: ${room.code}`, url: link })}>{t("Share")}</button>}</div></div></section>;
}
